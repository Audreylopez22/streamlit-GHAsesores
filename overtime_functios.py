...
def calculate_night_surcharges_hours(hour_rate,entry_datetime,worked_hours, day_key, holidays, total_week_hours):
...
def calculate_day_surcharges_hour(hour_rate,entry_datetime,worked_hours, day_key, holidays, total_week_hours):
    holidays_surchage_hour = 0
    if day_key in holidays:
        for hour_day in rrule.rrule(rrule.HOURLY,dtstart=entry_datetime,until=entry_datetime+ timedelta(hours=worked_hours)):
            if total_week_hours > config.getint(period, 'max_hours_per_week'):
                continue
            elif hour_day.hour > config.getint(period, 'end_night_limit') and hour_day.hour < config.getint(period, 'start_night_limit'):
...
def calculate_night_overtime_and_holydays(hour_rate,entry_datetime,worked_hours, day_key, holidays, total_week_hours):
    ...
    if day_key != 10 and day_key != 12 and day_key not in holidays:
        for hour_day in rrule.rrule(rrule.HOURLY,dtstart=entry_datetime,until=entry_datetime+ timedelta(hours=worked_hours)):
            ...
            elif night_overtime_hours >0 and hour_day.hour < config.getint(period, 'end_night_limit'):
                night_overtime_hours+= 1
    ...
    night_overtime_pay = round(night_overtime_hours*(hour_rate+(hour_rate*config.getfloat(period, 'night_overtime'))))
    night_holidays_overtime_pay = round(night_holidays_overtime_hours*(hour_rate+(hour_rate*config.getfloat(period, 'night_overtime'))))            
        
        
    return night_overtime_pay,night_overtime_hours, night_holidays_overtime_pay,night_holidays_overtime_hours

# calculo de las horas extras diurnas en dom-fes
def calculate_daytime_holidays_hour(hour_rate,entry_datetime,worked_hours, day_key, holidays,total_week_hours):
    ...
    daytime_holidays_pay = round(daytime_holidays_hours*(hour_rate+(hour_rate*config.getfloat(period, 'daytime_holidays'))))
        
    return  daytime_holidays_hours, daytime_holidays_pay

# calculo de horas extra normal
def calculate_daytime_overtime(hour_rate,entry_datetime,worked_hours, day_key, holidays, total_week_hours):
    ...
    daytime_overtime_pay = round(daytime_overtime_hours*(hour_rate+(hour_rate*config.getfloat(period, 'daytime_overtime'))))
